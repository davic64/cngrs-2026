...
import { MapPin, Navigation, Car, Coffee, Wifi, Shield, ExternalLink } from "lucide-react";
import * as motion from "framer-motion/client"; // Usando cliente de motion directamente en server components si es posible o simplemente div
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col p-4 sm:p-8 pb-32 md:pb-12">
      <header className="mb-10 text-center md:text-left">
        <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-1">Ubicación del Evento</p>
          ...
          <DashboardCard className="relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="space-y-2">
                <h2 className="text-2xl font-black text-secondary uppercase tracking-tighter">{venue.name}</h2>
                <p className="text-sm text-gray-500 font-medium leading-relaxed">{venue.address}</p>
              </div>
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                <a href={venue.mapsUrl} target="_blank" rel="noopener noreferrer" className="flex-1 sm:flex-none">
                ...
                {venue.websiteUrl && (
                  <a href={venue.websiteUrl} target="_blank" rel="noopener noreferrer" className="flex-1 sm:flex-none">
                    <Button variant="outline" className="w-full h-12 px-8 uppercase font-black text-[10px] tracking-widest">
        ...
        <div className="lg:col-span-5">
          <div className="bg-gray-200 rounded-[2rem] h-[350px] lg:h-full w-full relative overflow-hidden group shadow-2xl border-4 border-white">
            ...
            <div className="absolute bottom-6 right-6 flex flex-col gap-2">
               <button type="button" className="h-10 w-10 bg-white rounded-xl shadow-lg flex items-center justify-center text-secondary font-black hover:bg-gray-50 cursor-pointer border border-gray-100 transition-transform active:scale-95">+</button>
               <button type="button" className="h-10 w-10 bg-white rounded-xl shadow-lg flex items-center justify-center text-secondary font-black hover:bg-gray-50 cursor-pointer border border-gray-100 transition-transform active:scale-95">-</button>
            </div>
          </div>
        </div>
