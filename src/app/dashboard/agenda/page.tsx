        ...
        <aside className="lg:col-span-4 space-y-6">
          <DashboardCard title="Filtros">
            <div className="space-y-6">
              ...
              />
              <div className="pt-2">
                <Button variant="outline" className="w-full h-11 font-bold uppercase text-[10px] tracking-widest">
                  Limpiar Filtros
                </Button>
              </div>
            </div>
          ...
          <div className="hidden lg:block bg-primary/5 border border-primary/10 rounded-[2rem] p-6">
            <div className="flex items-center gap-3 mb-3 text-primary">
              ...
              <h3 className="font-black uppercase tracking-tighter text-sm">Favoritos</h3>
            </div>
            <p className="text-[11px] text-secondary/60 font-medium leading-relaxed uppercase tracking-wider">
              Marca las actividades con una estrella para armar tu propia agenda personalizada.
            </p>
          </div>
        </aside>
            ...
            >
              {filteredEvents.length > 0 ? (
                ...
                >
                  ...
                  <p className="font-black text-secondary uppercase tracking-tighter">No hay resultados</p>
                  <p className="text-xs text-gray-400 mt-1 uppercase tracking-widest font-bold">Intenta cambiar tus filtros</p>
                </motion.div>
              )}
            </motion.div>
          </AnimatePresence>
...
function EventItemCard({ title, speaker, time, location, category, isFavorite }: {
  title: string;
  speaker: string;
  time: string;
  location: string;
    ...
    <div className="bg-white rounded-[2rem] p-6 shadow-xl shadow-black/[0.03] border border-gray-100 group hover:border-primary/30 transition-all flex flex-col sm:flex-row gap-6 items-start sm:items-center">
      ...
      <div className="flex-1 space-y-2 text-left">
        <div className="flex items-center gap-2">
          ...
          {isFavorite && <Star size={14} className="text-primary fill-primary" />}
        </div>
      ...
      <div className="w-full sm:w-auto pt-4 sm:pt-0 border-t sm:border-t-0 border-gray-50 flex justify-end">
        <Button variant="outline" size="sm" className="w-full sm:w-auto h-10 px-6 font-bold uppercase text-[10px] tracking-widest">
          Detalles
        </Button>
      </div>
    </div>
