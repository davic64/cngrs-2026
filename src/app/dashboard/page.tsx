    ...
    <div className="w-full max-w-6xl mx-auto flex flex-col p-4 sm:p-8 pb-32 md:pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
        <div className="lg:col-span-5 space-y-6 lg:space-y-8">
          ...
          >
            <h2 className="text-sm font-black text-secondary/40 uppercase tracking-[0.2em] mb-6">Pase de Acceso Digital</h2>
            
            <div
              role="button"
              tabIndex={0}
              onClick={() => setIsQRModalOpen(true)}
              onKeyDown={(e) => (e.key === "Enter" || e.key === " ") && setIsQRModalOpen(true)}
            ...
            >
              <QrCode size={180} className="text-secondary group-hover:text-primary transition-colors" />
            </div>
            
            <div className="space-y-1 mb-8">
              <p className="text-2xl font-black text-secondary uppercase tracking-tighter">{user.id}</p>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Muestra este código al ingresar</p>
            </div>
          ...
          <button
            ...
            className={cn(
              ...
              currentStatus === 'validacion' ? "bg-gray-50 border-gray-200 text-gray-600" :
          ...
          >
            <div className={cn(
              ...
              currentStatus === 'parcial' ? "bg-white text-blue-600" :
              currentStatus === 'validacion' ? "bg-white text-gray-400" :
              "bg-white text-amber-600"
            )}>
              {currentStatus === 'completado' ? <CheckCircle2 size={24} /> : 
               currentStatus === 'validacion' ? <History size={24} /> :
               <AlertCircle size={24} />}
            </div>
            <div className="flex-1">
              <p className="text-[10px] font-black uppercase tracking-widest opacity-60">Estatus de Registro</p>
              <p className="font-black uppercase tracking-tighter text-lg leading-none">
                {currentStatus === 'completado' ? 'Validado y Pagado' : 
                 currentStatus === 'parcial' ? 'Parcialmente Pagado' : 
                 currentStatus === 'validacion' ? 'Validación Pendiente' : 'Pendiente de Pago'}
              </p>
            </div>
        ...
        <div className="lg:col-span-7 space-y-6 lg:space-y-8">
          ...
          <div className="bg-secondary rounded-[2rem] p-6 text-white/90 flex items-start gap-4 shadow-xl">
             ...
             <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-primary mb-1">Tip del Congreso</p>
                <p className="text-xs leading-relaxed font-medium">Llega 15 minutos antes a tus talleres preferidos, el cupo es limitado y se llena rápido.</p>
             </div>
          </div>
        </div>
      </div>
      ...
      <Modal isOpen={isQRModalOpen} onClose={() => setIsQRModalOpen(false)}>
        <ModalHeader onClose={() => setIsQRModalOpen(false)}>
          <ModalTitle className="text-2xl font-black uppercase tracking-tighter text-center">Pase de <span className="text-primary">Acceso</span></ModalTitle>
          <ModalDescription className="text-center font-bold uppercase text-[10px] tracking-widest mt-2">ID: {user.id}</ModalDescription>
        </ModalHeader>
        <ModalContent className="flex flex-col items-center justify-center py-8">
          ...
          <p className="font-black text-secondary text-3xl tracking-tighter uppercase">{user.id}</p>
        </ModalContent>
        <ModalFooter>
          <Button onClick={() => setIsQRModalOpen(false)} className="w-full h-12 uppercase font-black text-xs tracking-widest">Cerrar</Button>
        </ModalFooter>
      </Modal>
      ...
      <Modal isOpen={isStatusModalOpen} onClose={() => setIsStatusModalOpen(false)}>
        ...
        <ModalContent className="space-y-6 pt-2">
          ...
          {currentStatus === 'completado' && (
            <div className="flex flex-col items-center text-center space-y-4 py-4">
              ...
              <h3 className="text-xl font-black text-secondary uppercase tracking-tighter">¡Todo en Orden!</h3>
              <p className="text-sm text-gray-500 font-medium">Tu registro ha sido validado correctamente.</p>
            </div>
          )}
          ...
          {currentStatus === 'validacion' && (
            <div className="flex flex-col items-center text-center space-y-4 py-4">
              ...
              <h3 className="text-xl font-black text-secondary uppercase tracking-tighter">En Validación</h3>
              <p className="text-sm text-gray-500 font-medium">Hemos recibido tu comprobante. Nuestro staff lo validará en un lapso de 24 a 48 horas. Recibirás un aviso por SMS.</p>
            </div>
          )}
            ...
            <div className="space-y-6">
              <div className="p-5 bg-amber-50 border border-amber-100 rounded-2xl space-y-3">
                <div className="flex gap-3 items-start">
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase text-amber-700 tracking-widest mb-1">Tiempo Restante para Pagar</p>
                    <CountdownDisplay type={user.metodoPago} />
                  </div>
                </div>
                <p className="text-[10px] font-bold text-amber-900/60 uppercase leading-tight">
                  Seleccionaste pago vía {user.metodoPago === 'transferencia' ? 'SPEI / Transferencia' : 'Efectivo'}. 
              ...
              <div className="bg-red-50 border border-red-100 p-4 rounded-2xl flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="text-[10px] font-black text-red-700 uppercase leading-tight">Límite de precio actual: {user.fechaLimitePrecio}</p>
                  <p className="text-[9px] font-bold text-red-600 uppercase mt-0.5 opacity-70">El costo subirá $200 MXN después de esta fecha.</p>
                </div>
              </div>

              <div className="flex justify-between items-center px-2">
                <span className="text-[10px] font-black text-secondary uppercase tracking-widest">Monto a Pagar:</span>
                <span className="text-xl font-black text-primary">${user.montoPendiente} MXN</span>
              </div>

              <div className="space-y-3">
                <p className="text-[10px] font-black text-secondary uppercase tracking-widest ml-1">Subir Comprobante</p>
                <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-200 rounded-3xl cursor-pointer hover:bg-gray-50 transition-all group">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                      {paymentFile ? paymentFile.name : "Tomar foto o elegir archivo"}
                    </p>
                  </div>
        ...
        <ModalFooter>
          ...
          {(currentStatus === 'validacion' || currentStatus === 'completado') && (
            <Button className="w-full h-12 uppercase font-black text-xs tracking-widest" onClick={() => setIsStatusModalOpen(false)}>Cerrar</Button>
          )}
        </ModalFooter>
      </Modal>
    </div>
